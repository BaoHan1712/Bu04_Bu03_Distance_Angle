...
static dwt_config_t config = {
        ...
        9,               /* Tx???. TX preamble code. Used in TX only. */
        9,               /* Rx???. RX preamble code. Used in RX only. */
        1,               /* ??????. 0 to use standard 8 symbol SFD, 1 to use non-standard 8 symbol, 2 for non-standard 16 symbol SFD and 3 for 4z 8 symbol SDF type */
        DWT_BR_6M8,      /* ????. Data rate. */
        DWT_PHRMODE_STD, /* ??????. PHY header mode. */
        DWT_PHRRATE_STD, /* ??????. PHY header rate. */
        (129 + 8 - 8),   /* ??????. SFD timeout (preamble length + 1 + SFD length - PAC size). Used in RX only. */
        (DWT_STS_MODE_1 | DWT_STS_MODE_SDC), /* STS??. STS enabled */
        DWT_STS_LEN_256, /* STS??. STS length see allowed values in Enum dwt_sts_lengths_e */
...
int simple_tx_pdoa(void)
{
    ...
    /* ??DW3000??/?????. Enabling LEDs here for debug so that for each TX the D1 LED will flash on DW3000 red eval-shield boards. */
    dwt_setleds(DWT_LEDS_ENABLE | DWT_LEDS_INIT_BLINK) ;

    /* ??DW3000????. Configure DW IC. See NOTE 5 below. */
    ...
    /* ??DW3000??????. Configure the TX spectrum parameters (power, PG delay and PG count) */
    dwt_configuretxrf(&txconfig_options);

    /* Loop forever sending frames periodically. */
    while(1)
    {
        /* ????????DW3000????. Write frame data to DW IC and prepare transmission. See NOTE 3 below.*/
        ...
        /* ??DW3000??????. Poll DW IC until TX frame sent event set. See NOTE 4 below.
        ...
        /* ??TX_DELAY_MS. Execute a delay between transmissions. */
        Sleep(TX_DELAY_MS);
 ...
 *    DW IC during its manufacture. However there is no guarantee this will not conflict with someone else? implementation. We recommended that
